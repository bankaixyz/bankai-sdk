...
impl ExecutionFetcher {
    ...
    pub async fn fetch_tx_proof(&self, tx_hash: FixedBytes<32>) -> SdkResult<TxProof> {
        let rpc_url: Url = self.rpc_url.parse().map_err(|e| SdkError::Provider(format!("invalid rpc url: {e}")))?;
    ...
    pub async fn fetch_tx_block_number(&self, tx_hash: FixedBytes<32>) -> SdkResult<u64> {
        ...
        let receipt = provider
            ...
            .await
            .map_err(|_| SdkError::NotFound(format!("block not found")))?
            .ok_or(SdkError::NotFound(format!("block not found")))?;   
