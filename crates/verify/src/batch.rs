...
use bankai_types::results::op_stack::OpStackResults;
...
use crate::op_stack::OpStackVerifier;
...
pub fn verify_batch_proof(wrapper: ProofBundle) -> Result<BatchResults, VerifyError> {
    ...
    if let Some(op_stack) = &wrapper.op_stack_proofs {
        for proof in &op_stack.header_proofs {
            let snapshot = OpStackVerifier::verify_header_proof(
                proof,
                bankai_block.op_chains.root,
                wrapper.hashing_function,
            )?;
            batch_results.op_stack.verified_snapshots.push(snapshot);
        }
    }
