    ...
    pub fn from_verication_output(output: &VerificationOutput) -> Self {
        ...
        Self {
            ...
            beacon: BeaconClient {
                ...
                header_root: bytes32_from_limbs(&output[2].to_bytes_be()[16..], &output[3].to_bytes_be()[16..]),
                justified_height: output[4].try_into().unwrap(),
                finalized_height: output[5].try_into().unwrap(),
                num_signers: output[6].try_into().unwrap(),
                mmr_root_keccak: bytes32_from_limbs(&output[7].to_bytes_be()[16..], &output[8].to_bytes_be()[16..]),
                mmr_root_poseidon: FixedBytes::from_slice(output[9].to_bytes_be().as_slice()),
                current_committee_hash: bytes32_from_limbs(&output[10].to_bytes_be()[16..], &output[11].to_bytes_be()[16..]),
                next_committee_hash: bytes32_from_limbs(&output[12].to_bytes_be()[16..], &output[13].to_bytes_be()[16..]),
            },
            execution: ExecutionClient {
                block_number: output[14].try_into().unwrap(),
                header_hash: bytes32_from_limbs(&output[15].to_bytes_be()[16..], &output[16].to_bytes_be()[16..]),
                justified_height: output[17].try_into().unwrap(),
                finalized_height: output[18].try_into().unwrap(),
                mmr_root_keccak: bytes32_from_limbs(&output[19].to_bytes_be()[16..], &output[20].to_bytes_be()[16..]),
